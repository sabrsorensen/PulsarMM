use super::apply::{
    ...
    maybe_backup_live_mxml_with, write_profile_snapshot_with, ApplyProfileDeps,
    SaveActiveProfileDeps, save_active_profile_command_entry_with,
};
use super::{apply_ops, engine, get_profiles_dir};
use crate::installation_detection::find_game_path;
use crate::mods::archive::extract_archive;
use crate::{get_downloads_dir, get_library_dir, settings_paths};
use std::fs;
...
pub fn save_active_profile_impl(app: &AppHandle, profile_name: &str) -> Result<(), String> {
    ...
    let write_profile_json = |path: &std::path::Path, content: &str| {
    ...
    let deps = SaveActiveProfileDeps {
        ...
        collect_profile_map_and_metadata: Box::new(crate::profiles::apply_logic::collect_profile_map_and_metadata),
        ...
        write_profile_snapshot: Box::new(|name, json_path, entries| write_profile_snapshot_with(
            name,
            json_path,
            entries,
            &write_profile_json,
...
pub async fn apply_profile_impl(app: &AppHandle, profile_name: &str) -> Result<(), String> {
    let read_profile_json =
        |path: &std::path::Path| fs::read_to_string(path).map_err(|_| "Profile not found".to_string());
    ...
    let mut extract_archive_with_progress =
        |archive_path: &std::path::Path,
         ...
         progress_cb: &mut dyn FnMut(u64)| { extract_archive(archive_path, library_mod_path, progress_cb) };
    let mut deploy_profile_entry = apply_ops::deploy_profile_entry;

    let mut deps = ApplyProfileDeps {
        get_profiles_dir: Box::new(|| get_profiles_dir(app)),
        load_profile_json_content: Box::new(|json_path| load_profile_json_content_with(json_path, &read_profile_json)),
        ...
        apply_profile_entries: Box::new(|profile_data, downloads_dir, library_dir, mods_dir| apply_profile_entries_with(
            profile_data,
            downloads_dir,
            library_dir,
            mods_dir,

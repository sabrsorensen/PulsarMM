...
fn default_config_sets_expected_fields() {
    ...
    assert!(cfg.custom_game_path.is_none());
...
fn parse_config_or_default_parses_valid_json_and_falls_back_on_invalid() {
    let parsed = parse_config_or_default(
        r#"{"custom_download_path":"/tmp/dl","custom_library_path":"/tmp/lib","custom_game_path":"/tmp/game","legacy_migration_done":true}"#,
        false,
    );
    assert_eq!(parsed.custom_download_path.as_deref(), Some("/tmp/dl"));
    assert_eq!(parsed.custom_library_path.as_deref(), Some("/tmp/lib"));
    assert_eq!(parsed.custom_game_path.as_deref(), Some("/tmp/game"));
    ...
    assert!(fallback.custom_game_path.is_none());
}
...
fn load_config_returns_default_when_existing_path_is_directory() {
    ...
    assert!(loaded.custom_game_path.is_none());
...
fn save_and_load_config_roundtrip() {
    ...
    cfg.custom_game_path = Some("/tmp/game".to_string());
    ...
    assert_eq!(loaded.custom_game_path.as_deref(), Some("/tmp/game"));
    assert!(loaded.legacy_migration_done);
